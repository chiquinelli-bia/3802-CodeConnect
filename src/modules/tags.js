        ...
        if (tagTxt !== "") {
          try {
            ...
            if (tagExiste) {
              ...
              newTag.innerHTML = `<p>${tagTxt}</p> <img src="./src/img/close-black.svg" class="remove-tag">`;
              ...
              alert(
                "Tag não foi encontrada. tags disponíveis: front-end, programação, data science, full-stack, html, css, javascript."
              );
            }
          } catch (error) {
            console.error("Erro ao verificar a existência da tag", error);
            alert(
              "Erro ao verificar a existência da tag. Verifique o console."
            );
          }
        }
      }
